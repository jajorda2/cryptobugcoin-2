...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "CryptoBugCoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"cryptobugcoin.com", "165.227.222.8"}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. CryptoBugCoin is probably already running."), addrBind.ToString().c_str());
        else
